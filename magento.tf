...
resource "terraform_data" "magento_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "su -c \"repo_user=${var.magento_pub_key} repo_pass=${var.magento_priv_key} base_url='${var.site_name}.freetls.fastly.net' service_id=${fastly_service_vcl.demo_service.id} api_key=${var.fastly_api_key} /usr/local/bin/magento.sh\" - magento_user"
    ]
  }
